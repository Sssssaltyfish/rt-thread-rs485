...
static const char* cmd_info[] = {
    "Usage: \n",
    "rs485 create [serial] [baudrate] [parity] [pin] [level] - create rs485 "
    "instance.\n",
    "rs485 destroy                                           - destroy rs485 "
    "instance.\n",
    "rs485 set_recv_tmo [tmo_ms]                             - set recieve "
    "timeout.\n",
    "rs485 set_byte_tmo [tmo_ms]                             - set byte "
    "timeout.\n",
    "rs485 connect                                           - open rs485 "
    "connect.\n",
    "rs485 disconn                                           - close rs485 "
    "connect.\n",
    "rs485 recv [size]                                       - receive from "
    "rs485.\n",
    "rs485 send [size]                                       - send to "
    "rs485.\n",
    "rs485 cfg [baudrate] [databits] [parity] [stopbits]     - config rs485.\n",
    "rs485 send_then_recv [send_size] [recv_size]            - send to rs485 "
    "and then receive from rs485.\n",
    "\n"
};
...
static void rs485_test(int argc, char** argv) {
    ...
    if (strcmp(argv[1], "create") == 0) {
        ...
        if (test_hinst != NULL) {
            rt_kprintf(
                "the test instance is not NULL, please deinit/destroy first.\n"
            );
            return;
        }
        if (argc >= 3) {
    ...
    if (strcmp(argv[1], "send_then_recv") == 0) {
        ...
        rt_kprintf(
            "rs485 send %d datas, then receive max length : %d .\n",
            send_size,
            recv_size
        );
        len = rs485_send_then_recv(
            test_hinst, test_buf, send_size, test_buf, recv_size
        );
        if (len == 0) {
            rt_kprintf("rs485 receive timeout.\n");
            return;
...
MSH_CMD_EXPORT_ALIAS(write, write, write string to a device);
#endif
